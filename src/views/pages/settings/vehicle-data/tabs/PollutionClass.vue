...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/pollution/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="pollutions?.pollutions || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Pollution" message="Sind Sie sicher, dass Sie dies entfernen möchten Pollution?"></ConfirmDialog>
</template>
