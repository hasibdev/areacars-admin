...
const fields = [
  {
    field: "gearshift_name",
    label: "Liste der Getriebe",
  },
  {
    field: "actions",
    label: "Aktionen",
...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/gearbox/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="gearboxs?.gearboxs || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Getriebe löschen" message="Möchten Sie dieses Getriebe wirklich entfernen?"></ConfirmDialog>
</template>
