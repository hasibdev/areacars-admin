...
const fields = [
  {
    field: "upholsterycolors",
    label: "Liste der Polsterfarbe",
  },
  {
    field: "has_code",
    label: "Farbcode",
...
const {
  data: upholsterycolor,
  loading,
  onApiDelete,
  reFetch,
  deleting,
} = useAxios("/admin/upholsterycolor")
...
const onDelete = async () => {
  try {
    await onApiDelete(
      `/admin/upholsterycolor/delete/${activeDeleteId.value}`
    )
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="upholsterycolor?.upholsterycolor || []" :fields="fields">
      <template #color="{ row }">
        <div class="color-box" :style="`background-color: #${row.has_code};`"></div>
      </template>
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Polsterfarbe löschen" message="Möchten Sie diese Polsterfarbe wirklich entfernen?"></ConfirmDialog>
</template>
