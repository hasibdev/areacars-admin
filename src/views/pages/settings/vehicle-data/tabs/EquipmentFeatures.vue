...
const fields = [
  ...
  },
  {
    ...
    handler: row => eqOptions.find(op => op.value == row.type).label
  },
  {
    field: "actions",
    label: "Aktionen",
...
const {
  data: equipmentFeatures,
  loading,
  onApiDelete,
  reFetch,
  deleting,
} = useAxios("/admin/equipmentfeature")
...
const onDelete = async () => {
  try {
    await onApiDelete(
      `/admin/equipmentfeature/delete/${activeDeleteId.value}`
    )
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="equipmentFeatures?.equipmentFeatures || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Ausstattungsmerkmal" message="Sind Sie sicher, dass Sie dies entfernen möchten Ausstattungsmerkmal?"></ConfirmDialog>
</template>
