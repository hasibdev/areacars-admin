...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/damage/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="damaged?.damaged || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Land" message="Sind Sie sicher, dass Sie dies entfernen möchten Land?"></ConfirmDialog>
</template>
