...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/condition/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="conditions?.conditions || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Zustand" message="Sind Sie sicher, dass Sie dies entfernen möchten Zustand?"></ConfirmDialog>
</template>
