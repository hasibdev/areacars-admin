...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/transmission/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="transmission?.transmission || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Transmission" message="Sind Sie sicher, dass Sie dies entfernen möchten Transmission?"></ConfirmDialog>
</template>
