...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/area/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="areaList?.areas || []" :fields="fields">
  ...
  <!-- Delete Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Bereich löschen" message="Möchten Sie diesen Bereich wirklich entfernen?"></ConfirmDialog>
</template>
