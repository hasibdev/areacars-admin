...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/drivable/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="drivables?.drivables || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Still Driveable" message="Sind Sie sicher, dass Sie dies entfernen möchten Still Driveable?"></ConfirmDialog>
</template>
