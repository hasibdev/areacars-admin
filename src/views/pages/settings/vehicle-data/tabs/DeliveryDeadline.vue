...
const {
  data: deliverydeadlines,
  loading,
  onApiDelete,
  reFetch,
  deleting,
} = useAxios("/admin/deliverydeadline")
...
const onDelete = async () => {
  try {
    await onApiDelete(
      `/admin/deliverydeadline/delete/${activeDeleteId.value}`
    )
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="deliverydeadlines?.deliverydeadlines || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Lieferfrist" message="Sind Sie sicher, dass Sie dies entfernen möchten Lieferfrist?"></ConfirmDialog>
</template>
