...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/model/delete/${activeDeleteId.value}`)
    reFetch()
...
watch(() => selectedBrand.value, (brandId) => {
  reFetch(`/admin/model/search?brand_id=${brandId}&model_name=${modelSearch.value}`)
})

watch(() => modelSearch.value, _.debounce(() => {
  reFetch(`/admin/model/search?brand_id=${selectedBrand.value}&model_name=${modelSearch.value}`)
}, 500))
</script>
...
<template>
  <div class="mt-4">
    <div class="row">
      <div class="col-md-6">
        <SelectControl v-model="selectedBrand" :options="brands?.brans || []" optionLabel="brand_name" label="Hersteller" />
      </div>
      <div class="col-md-6">
        <InputControl v-model="modelSearch" :disabled="!selectedBrand" placeholder="Search" />
      </div>
    </div>

    <DataTable :loading="loading" :data="models?.data || []" :fields="fields">
      ...
      <template #brand_images="{ row }">
        <div class="d-flex">
          <div>
            <img :src="getImgUrl(`public/brand_logo/${row.brand_images}`)" class="rounded" style="width: 80px; aspect-ratio: 1.2/1" />
          </div>
        </div>
      </template>
    </DataTable>
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Hersteller löschen" message="Möchten Sie diesen Hersteller wirklich entfernen?"></ConfirmDialog>
</template>
