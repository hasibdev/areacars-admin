...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/color/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="colors?.colors || []" :fields="fields">
      ...
      <template #color="{ row }">
        <div class="color-box" :style="`background-color: #${row.color_code};`"></div>
      </template>
    </DataTable>
  </div>

  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Außenfarbe löschen" message="Möchten Sie diese Außenfarbe wirklich entfernen?"></ConfirmDialog>
</template>
