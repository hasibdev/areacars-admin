...
const {
  ...
  reFetch,
  deleting,
} = useAxios("/admin/energyefficiency")
...
const onDelete = async () => {
  try {
    await onApiDelete(
      `/admin/energyefficiency/delete/${activeDeleteId.value}`
    )
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="energyefficiency?.energyefficiency || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Energieeffizienz" message="Sind Sie sicher, dass Sie dies entfernen möchten Energieeffizienz?"></ConfirmDialog>
</template>
