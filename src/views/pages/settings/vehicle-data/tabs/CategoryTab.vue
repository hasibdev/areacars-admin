...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/category/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="categories || []" :fields="fields">
      ...
      <template #icon="{ row }">
        <div class="d-flex">
          <div>
            <img :src="getImgUrl(`public/icon/${row.icon}`)" class="rounded" style="width: 80px" />
          </div>
        </div>
      </template>
    </DataTable>
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Kategorie löschen" message="Möchten Sie diese Kategorie wirklich entfernen?"></ConfirmDialog>
</template>
