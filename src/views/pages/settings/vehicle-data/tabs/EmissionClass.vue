...
const onDelete = async () => {
  try {
    await onApiDelete(`/admin/emissionclass/delete/${activeDeleteId.value}`)
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="emissions?.emissions || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Entfernen Emissionsklasse" message="Sind Sie sicher, dass Sie dies entfernen möchten Emissionsklasse?"></ConfirmDialog>
</template>
