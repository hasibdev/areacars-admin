...
const fields = [
  {
    field: "imaterial_name",
    label: "Liste des Innenmaterials",
  },
  {
    field: "actions",
    label: "Aktionen",
...
const {
  ...
  reFetch,
  deleting,
} = useAxios("/admin/interiormaterial")
...
const onDelete = async () => {
  try {
    await onApiDelete(
      `/admin/interiormaterial/delete/${activeDeleteId.value}`
    )
    reFetch()
...
<template>
  <div class="mt-4">
    <DataTable :loading="loading" :data="interiormaterials?.interiormaterial || []" :fields="fields">
  ...
  <!-- Entfernen Modal -->
  <ConfirmDialog v-model="showDeleteModal" @onOk="onDelete" :okLoading="deleting" title="Innenmaterial löschen" message="Möchten Sie dieses Innenmaterial wirklich entfernen?"></ConfirmDialog>
</template>
