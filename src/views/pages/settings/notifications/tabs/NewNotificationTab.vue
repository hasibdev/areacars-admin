...
const activeAndroidPhone = ref(false)
...
// Search car
const carsData = ref([])
watch(() => form.model_id, async (modelId) => {
  try {
    const res = await api.get(`/admin/push_notifactions/getmodelwisevichle/${modelId}`)
    carsData.value = res.data.carlist
  } catch (error) {
    //
  }
})
...
<template>
  <div class="border-top mt-4">
    <div class="row pt-2">
      <div class="col-md-5">
        <InputControl v-model="form.title" label="Titel der Benachrichtigung" class="mt-3" errorField="title" />
        <TextControl v-model="form.description" label="Nachricht" class="mt-3" errorField="description" />

        <!-- <SelectControl label="Fahrzeug (optional)" v-model="form.car_id" :options="productList?.data || []" trackBy="car_id" optionLabel="brand_name" class="mt-3" errorField="car_id" /> -->

        <SearchableSelect v-model="form.car_id" trackBy="car_id" :handleSelect="customSelect" label="Fahrzeug" :canAdd="false" optionLabel="brand_name" :dataConfig="modelWiseVehicle" errorField="car_id" class="mt-3">
      ...
      <div class="col-md-5 offset-md-2">
        <div class="d-flex justify-content-between align-items-start mt-3">
          ...
          <div class="phone__box">
            <img v-if="!activeAndroidPhone" src="@/assets/svg/phonee-notif-02.svg" alt="" class="phone__image" />
            <img v-else src="@/assets/svg/phonee-notif-03.svg" alt="" class="phone__image" />

            <div class="text_box">
              <p class="title text-truncate">{{ form.title }}</p>
          ...
          <div class="d-flex">
            <div @click="activeAndroidPhone = true" :class="{ 'border-primary': activeAndroidPhone }" class="pointer px-2 py-1 border rounded d-flex justify-content-center align-items-center">
            ...
            <div @click="activeAndroidPhone = false" :class="{ 'border-primary': !activeAndroidPhone }" class="pointer px-2 py-1 border rounded ms-2">
        ...
        <p class="text-center w-75 mx-auto mt-3">
          *Eine Push-Benachrichtigung wird an Ihre Kunden gesendet, wenn sie ihre Einstellungen aktiviert haben.
        </p>
      </div>
    </div>
    ...
    <div class="d-flex align-items-center justify-content-end mt-5 pt-5">
      <PrimaryButton @click="$router.push('/settings')" text="Abbrechen" variant="light"></PrimaryButton>
